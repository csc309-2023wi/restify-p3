...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        //   const access_token = localStorage.getItem('access_token');
          const access_token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlbl90eXBlIjoiYWNjZXNzIiwiZXhwIjoxNjgyMjk0MjkzLCJpYXQiOjE2ODIyMDc4OTMsImp0aSI6IjlhYjhkZWY5MWEyMjQ3OTFhMmM4OTM5Yjc1NGYyMjY4IiwidXNlcl9pZCI6NH0.1I7zQFuv6dArNCvYRlRl9PfgiTWRG_qEXjWDMFtP024";
          if (access_token) {
            try {
            const headers = {
            ...
            const reservationPromises = response.data.results.map(async (reservation) => {
                const propertyResponse = await axios.get(`http://localhost:8000/api/property/${reservation.property_id}`);
              return {
                  ...reservation,
      ...
      useEffect(() => {
        ...
        //   const access_token = localStorage.getItem('access_token');
          ...
          if (access_token) {
            ...
            const propertyResponse = await axios.get(`http://localhost:8000/api/property/?host_id=${response.data.id}`);
            // console.log(propertyResponse.data.results);
          const resolvedProperties = await Promise.all(propertyResponse.data.results);
                ...
                <header className="section-header">
                    ...
                    <label>
                        ...
                        <select name="state" value={filterState.state} onChange={handleFilterChange}>
                            ...
                            <option value="Pending" onClick={handleFilterChange}>Pending</option>
                            <option value="Denied" onClick={handleFilterChange}>Denied</option>
                            <option value="Expired" onClick={handleFilterChange}>Expired</option>
                            <option value="Approved" onClick={handleFilterChange}>Approved</option>
                            <option value="Completed" onClick={handleFilterChange}>Completed</option>
                            <option value="Canceled" onClick={handleFilterChange}>Canceled</option>
                            <option value="Terminated" onClick={handleFilterChange}>Terminated</option>
                        </select>
                    </label>
                </header>
